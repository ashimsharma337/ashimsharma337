# Hi, I'm Ashim Sharma 👋

### Full-Stack Software Engineer

I build scalable web applications, backend services, and APIs using modern JavaScript, Python, and Java ecosystems.

I'm particularly interested in **backend engineering, API design, PostgreSQL, cloud infrastructure, and scalable application architecture**.

---

## 🚀 What I Work With

### Frontend

<p>
<img src="https://img.shields.io/badge/Next.js-000000?style=flat&logo=next.js&logoColor=white" />
<img src="https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black" />
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black" />
<img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white" />
<img src="https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white" />
<img src="https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white" />
</p>

### Backend

<p>
<img src="https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white" />
<img src="https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white" />
<img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=springboot&logoColor=white" />
<img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white" />
<img src="https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white" />
</p>

### APIs & Data

<p>
<img src="https://img.shields.io/badge/REST-02569B?style=flat" />
<img src="https://img.shields.io/badge/GraphQL-E10098?style=flat&logo=graphql&logoColor=white" />
<img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white" />
<img src="https://img.shields.io/badge/SQLite-07405E?style=flat&logo=sqlite&logoColor=white" alt="SQLite" />
<img src="https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white" />
<img src="https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white" />
</p>

### Cloud & DevOps

<p>
<img src="https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws&logoColor=white" />
<img src="https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white" />
<img src="https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white" />
<img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat&logo=githubactions&logoColor=white" />
<img src="https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black" />
</p>

---

## 🧠 Engineering Interests

* Backend & API architecture
* REST and GraphQL APIs
* PostgreSQL and relational data modeling
* Full-stack application development
* Authentication & authorization
* Docker & Kubernetes
* AWS cloud architecture
* CI/CD and deployment automation
* Performance, scalability and reliability
* Distributed systems and asynchronous processing

---

## ⭐ Featured Projects

### 🛒 E-Commerce Platform

**Java · Spring Boot · React · REST · JWT · SQL**

Full-stack e-commerce application demonstrating authentication, product management, shopping cart, checkout workflows and REST API integration.

➡️ [View Project](https://github.com/ashimsharma337/ecommerce-frontend-backend)

---

### 🍽️ Restaurant Inventory Management

**Next.js** · **React** · **GraphQL (Apollo)** · **PostgreSQL** · **SQLite (FTS5)** · **Docker/K8s**

Inventory dashboard with a GraphQL API, full-text search cache, and Kubernetes deployment manifests.

➡️ [View Project](https://github.com/ashimsharma337/Restaurant-Inventory-Management)

---

### 🐍 Python / FastAPI Monorepo

**Python · FastAPI · PostgreSQL · SQL**

Backend-focused Python projects exploring API development, PostgreSQL integration, database design and scalable application structure.

➡️ [View Project](https://github.com/ashimsharma337/python-monorepo)

---

## 📊 GitHub

<!-- ![GitHub Stats](https://github-readme-stats.vercel.app/api?username=ashimsharma337&show_icons=true&hide_border=true&count_private=true) -->
![Streak Stats](https://github-readme-streak-stats.herokuapp.com/?user=ashimsharma337&hide_border=true)
<!-- ![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=ashimsharma337&layout=compact&hide_border=true) -->

![Profile Views](https://komarev.com/ghpvc/?username=ashimsharma337&color=blue&style=flat)

---

## 📚 Currently Learning & Building

* Advanced PostgreSQL
* FastAPI & Python backend development
* GraphQL API architecture
* Docker & Kubernetes
* AWS cloud architecture
* Scalable backend systems
* System design

---

## 🤝 Let's Connect

I'm interested in collaborating on interesting software engineering projects, backend systems, APIs and cloud-native applications.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=flat\&logo=linkedin)](YOUR_LINKEDIN_URL)

[![GitHub](https://img.shields.io/badge/GitHub-ashimsharma337-black?style=flat\&logo=github)](https://github.com/ashimsharma337)

---

> **Build it. Understand it. Improve it.**
